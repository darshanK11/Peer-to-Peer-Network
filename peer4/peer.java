...
public class peer {
    ...
    public static Dictionary peersPeer4 = new Hashtable();
    ...
    public static void main(String[] args) throws IOException {
        ...
        Thread thread1 = new Thread () {
            public void run () {
                try {
                    Socket socketPeer4Client = new Socket(InetAddress.getByName("localhost"), 7002);
                    DataInputStream dataInputStreamPeer4Client = new DataInputStream(socketPeer4Client.getInputStream());
                    DataOutputStream dataOutputStreamPeer4Client = new DataOutputStream(socketPeer4Client.getOutputStream());
                    while(true) {
                        String clientFiles = getFiles();
                        String serverFiles = dataInputStreamPeer4Client.readUTF();
                        String[] serverFileList = serverFiles.split("\n");
                        if(hasAllFiles()) {
                            dataOutputStreamPeer4Client.writeUTF("Exit");
                            System.out.println("Receieved all parts.\nClosing this connection.");
                            mergeAllParts();
                            socketPeer4Client.close();
                            System.out.println("Connection Closed.");
                            break;
                        }
                        for(String fileName: serverFileList) {
                            if(!clientFiles.contains(fileName)) {
                                ...
                                dataOutputStreamPeer4Client.writeUTF("Sending file " + fileName + " to Peer4");
                        ...
                        System.out.println(dataInputStreamPeer4Client.readUTF());
                    }
                    dataInputStreamPeer4Client.close();
                    dataOutputStreamPeer4Client.close();
                }
                catch(Exception e) {
                    e.printStackTrace();
                }
        ...
        Thread thread2 = new Thread () {
            public void run() {
                try {
                    ...
                    while(true) {
                        Socket socketPeer4Server = serverSocket.accept();
                        System.out.println("\nA new peer was connected.");
                        DataInputStream dataInputStreamPeer4Server = new DataInputStream(socketPeer4Server.getInputStream());
                        DataOutputStream dataOutputStreamPeer4Server = new DataOutputStream(socketPeer4Server.getOutputStream());
                        System.out.println("A new thread was allocated for this peer.");
                        peer.peersPeer4.put(socketPeer4Server.getPort(), "peer" + peerNumber);
                        peerNumber++;
                        Thread thread = new PeerHandler(socketPeer4Server, dataInputStreamPeer4Server, dataOutputStreamPeer4Server);
                        thread.start();
                    }
                }
                catch(Exception e) {
...
class PeerHandler extends Thread {
    ...
    public void run() {
        ...
        while(true) {
            try {
                String peerName = (String)peer.peersPeer4.get(socket.getPort());
                String serverFiles = getFiles();
                dataOutputStream.writeUTF(serverFiles);
                recieved = dataInputStream.readUTF();
                if (recieved.equals("Exit")) {
                    System.out.println((String)peer.peersPeer4.get(socket.getPort()) + " is exiting.");
                    this.socket.close();
                    socket.close();
                    System.out.println("Connection Closed.");
                    break;
